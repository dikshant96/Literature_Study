...
\end{equation}
For modelling the thermal variance dissipation, either a full transport equation for $\epsilon_{theta}$ is solved or an algebraic formulation based on a time scale ratio $R$. Based on the assumption that the ratio of thermal and dynamic time-scales is constant given in equation \ref{eq:scalar_timescale_ratio}, a simple algebraic expression for $\epsilon_{\theta}$ is proposed given in equation \ref{eq:scalar_dissipation_algebraic}. Typically, a constant value of $R = 0.5$ is assumed \cite{Leschziner2015}. 
\begin{equation}
